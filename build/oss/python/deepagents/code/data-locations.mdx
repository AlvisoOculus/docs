...
| **ChatGPT OAuth token** | `~/.deepagents/.state/chatgpt-auth.json` | R/W | Owner-only; backs the [`openai_codex`](/oss/python/deepagents/code/providers#sign-in-with-chatgpt-codex-models) provider, refreshed automatically |
