...
Selecting the `openai_codex` provider in `/auth` starts a browser-based OAuth flow instead of prompting for an API key, letting you use OpenAI's Codex models through a ChatGPT subscription. The resulting token is stored at `~/.deepagents/.state/chatgpt-auth.json` and refreshed automatically. To re-authenticate or sign out, select `openai_codex` again. See [Sign in with ChatGPT (Codex models)](/oss/python/deepagents/code/providers#sign-in-with-chatgpt-codex-models) for the full flow.
